...
import { FUND_ANALYTICS } from '../data/demoData';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ tickets, onRefreshTickets }) => {
  ...
  // Static demo mode for GitHub Pages (no Express backend)
  const fetchStats = async () => {
    setStats(FUND_ANALYTICS);
  };
  ...
  const handleSendExpertReply = async () => {
    ...
    try {
      // Static GitHub Pages demo: save response only in UI.
      if (selectedTicket) {
        setSelectedTicket({
          ...selectedTicket,
          status: 'resolved',
  ...
  const handleRunSimulator = async () => {
    ...
    try {
      // Static demo classifier (backend is unavailable on GitHub Pages).
      const data = {
        category: 'legal_basics',
        complexity: 'medium',
        recommendation: 'Провести консультацию и проверить документы.',
      };
      setSimResult(data);
    } catch (e: any) {
