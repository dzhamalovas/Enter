...
Задайте ваш вопрос или выберите один из частых сценариев ниже.`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      // GitHub Pages has no server-side API. Provide offline demo response.
      const aiMsg: ChatMessage = {
        id: `ai-${Date.now()}`,
        sender: 'ai',
        text: `Спасибо за вопрос. В демо-версии на GitHub Pages ИИ работает без серверного API.\n\nЯ рекомендую проверить юридическую форму бизнеса, налоги, доступные меры поддержки и подготовить план следующих шагов.`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
            ...
            >
              ...
              {msg.sender === 'ai' && !msg.analysis?.ticketCreated && msg.id !== 'welcome-msg' && (
                <div className="mt-4 pt-3 border-t border-blue-100 flex items-center justify-between text-xs">
                  ...
                  <button
                    onClick={() => {
                      // Static GitHub Pages demo: no backend request.
                      onTicketCreated?.(`demo-${Date.now()}`);
                      onNavigate('tickets');
                    }}
